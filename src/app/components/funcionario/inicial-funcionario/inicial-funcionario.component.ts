...
export class InicialFuncionarioComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.carregarPedidos(); // Carrega os pedidos na inicialização
  }

  carregarPedidos(): void {
    this.listarTodosPedidos(); // Carrega todos os pedidos
    this.filtrarPedidos(); // Filtra de acordo com o filtro atual
  }

  listarTodosPedidos(): void {
    this.pedidoService.listarTodosPedidos().subscribe(
      (data: Pedido[]) => {
        this.pedidos = data; // Atribui os dados recebidos à variável pedidos
        this.filtrarPedidos(); // Aplica o filtro após a carga dos pedidos
      },
      (error) => {
  ...
  lavarPedido($event: any, pedido: Pedido): void {
    ...
    // Alterar o status do pedido para "AGUARDANDO PAGAMENTO"
  ...
  private atualizarPedidos(): void {
    this.listarTodosPedidos(); // Recarrega a lista de pedidos
  }
